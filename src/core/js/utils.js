var Utils = function Utils(){
      ...
      if (new RegExp(/^2[\d][\d]/mg).test(request.status)) {
        ...
        if (contentType.includes('text/html')) 
          ...
          msg = request.statusText!=""? request.statusText:
            (fw.labels.errors[request.status]?fw.labels.errors[request.status][fw.lang]:"");
        reject({
          'status': 'error',
  ...
  /**
   * return a flattened version of an Object. Keys are formatted to be used in a FormData object (see utils.request)
   * @param  {Object} obj    
   * @param  {string} parent
   * @param  {Object} res    
   */
  ...
  utils.getObjBy = function(arrObj,property,value){
    ...
    if (Array.isArray(arrObj)) {
        ...
        items = utils.filterObj(arrObj, property, value);
  ...
  utils.filterObj = function (obj, property, value) {
   ...
   * @param  {String}  html [Content inserted in the iframe]
   * @return {Element} iframe
   */
  utils.appendIsolatedHTML = function(el,html){
  ...
  */
   ...
   */
  utils.checkForm = function(el,renderError = true,reportFormValidity = true){
    if (fw.debug) 
      fw.log('checkForm');
    if (el.nodeName == 'FORM')
      el.reportValidity()
    ...
    $.each(inputs, function(index, input){
      ...
      var valid = true;
        ...
        // FILEUPLOADER 
        if($(input).hasClass('fileUploader') && fw.components.includes('fileUploader')){
        ...
        // DATEPICKERS
        if($(input).hasClass('datepicker') && fw.components.includes('datepicker')){
            inputRow.value = $(input).attr('data-tstamp');
        }

        // WIZARDS
        if($(input).hasClass('wizard') && fw.components.includes('wizard')){
      ...
      $.each(results.inputs, function(index, input){
        if(!input.valid){
          ...
          if (input.value == false) // assuming input's value is empty / falsy
            labelError = fw.labels.errors.inputs.empty[fw.lang].replace('%s',labelError);
          else  // assuming input's value is incorrect
            labelError = fw.labels.errors.inputs.incorrect[fw.lang].replace('%s',labelError);
  ...
  utils.getInputLabel = function(inputID, placeholder=undefined){
  ...
  function notifCleanMsg(str){
    if (str.includes('</html>')){
      ...
      dial.showModal()
      utils.appendIsolatedHTML(dial,str);
      str = 'Error';
    }
    return str;
  }
